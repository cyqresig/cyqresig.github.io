...
let webpackConfig = {
    ...
    output: {
        ...
        */
        ...
        path: path.resolve(__dirname, "build"), //如使用webpack-dev-server, 则相对该目录的位置为web服务器的根目录
        ...
        // filename 指输出的每个js模块名称的引用路径模板定义，会影响在html中模块的引用路径
     ...
     devServer: {
         ...
         contentBase: path.resolve(__dirname, "build"), // 需要配置成编译后的模块文件存放的路径（同output中的path）
